...
def define_list_of_executed_instructions(module, official_starting_time, official_kernel_op_time):

	address_profiler_name = str(main_path) + "/intruction_level_ers/TB/" + str(module) + "/instruction_logger.log"
	sass_instructions_name = str(main_path) + "/intruction_level_ers/TB/4.4_FlexGripPlus/FlexGripPlus_Pro_ed/RTL/TB/TP/TP_instructions.vhd"
...
def execute_golden_sim(input_parameter, input_fault_model):
	...
	os.chdir(str(main_path) + "/intruction_level_ers/TB/" + str(input_parameter))
	...
	if (total_time != 0) and (totalcpu_time != 0) and (official_kernel_op_time != 0):
		...
		os.system("rm ../../../parallel/"  + str(folder_fault_model) + str(input_parameter) + "/DUT/RTL_sim/dumpports_" + str(input_parameter) + ".vcd")		# movement of the output report for the fault simulation environment....
		os.system("cp dumpports_" + str(input_parameter) + ".vcd ../../../parallel/"  + str(folder_fault_model) + str(input_parameter) + "/DUT/RTL_sim/dumpports_" + str(input_parameter) + ".vcd")
...
# ---------------------------------------------------------------------------------------------------------------------------
# ---------------------------------------------------------------------------------------------------------------------------
# This function collects the list of instructions and determines the times for the parallel fault simulation on the individual instructions:

def identify_instructions_to_threads(total_list):

	times_lists = []
	total_number_threads = 0

	for i in range(0, len(total_list)):
		if "config" not in total_list[i][0]:

			local_list = []
			local_list.append(total_list[i][0])		# Instruction name
			local_list.append(total_list[i][2])		# Starting time
			local_list.append(total_list[i][3])		# Finishing time
			
			times_lists.append(local_list)
			total_number_threads = total_number_threads + 1
	
	return(total_number_threads, times_lists)
...
def 	modify_strobes_files(i, analyzed_list, input_parameter, application_name):

	file_name = str(input_parameter) + "_" + application_name + "_" + str(i) + "/bin/strobes.sv"
	
	file_obj = open(file_name, 'r')
	new_file_obj = open(file_name + "_new", 'w')
	
	for line in file_obj:
		
		if "XXX_1" in line:
			new_file_obj.write("		#" + str( int(analyzed_list[1]) - 1 ) + ";			// XXX_1")
		elif "XXX_2" in line:
			new_file_obj.write( "		for (int j= " + str(analyzed_list[1]) + "; j<" + str(analyzed_list[2]) + "; j=j+2) begin            // XXX_2" )
		else:
	...
	os.system("rm " + str(file_name) )
	os.system("cp " + file_name + "_new" + " " + file_name )
	os.system("rm " + str(file_name) + "_new")
...
def collapse_reports(input_parameter, folder_fault_model, total_number_threads, application_name):
	
	for j in range(0, total_number_threads):
		os.chdir(str(main_path) + "/parallel/" + str(folder_fault_model) + input_parameter + "_" + application_name + "_" + str(j))
		os.system("cp saf_dic_long.txt ../" + str(input_parameter) + "/saf_dic_long" + str(j) + ".txt")
	
	os.chdir(str(main_path) + "/parallel/" + str(folder_fault_model) + input_parameter)
	
	string_comand = ""
	for k in range(0, total_number_threads):
		string_comand = string_comand +  "saf_dic_long" + str(k) + ".txt "
	
	complete_command = "cat " + string_comand + " > saf_dic_long.txt"
	print(complete_command)
	os.system(complete_command)
	
	# Remove the files used for the joining and merge of parallel results. (for now, maybe these can be used later for another purposes...)
#	for kk in range(0, total_number_threads):
#		os.system("rm saf_dic_long" + str(kk) + ".txt ")
...
def	execute_fault_injection_campaign(fault_injection_enable, input_parameter, input_fault_model, application_name, official_starting_time, official_kernel_op_time):
	...
	if fault_injection_enable == 1:
		...
		total_list = []  
		(total_list) = define_list_of_executed_instructions(input_parameter, official_starting_time, official_kernel_op_time)
		
		print(total_list)
				
		analyzed_list = []
		(total_number_threads, analyzed_list) = identify_instructions_to_threads(total_list)	# This function defines the total number of threads to simulate in zoixs as the number of instructions.
		
		print(analyzed_list)
		print("total number of parallel threads to generate: " + str(total_number_threads))

#		os.chdir(str(main_path) + "/parallel/" + str(folder_fault_model) + str(input_parameter))
 		os.chdir(str(main_path) + "/parallel/" + str(folder_fault_model))
 
		os.system("rm " + str(input_parameter) + "/saf_dic_long.txt")				# Removal of any previous fault from a fault injection with a different application.
	
				
		# launch the simulations, one per time in sequence or parallel.
		
		
		timestamp4 = time.time()

#		# method number one: sequential... not efficient and stopped its development
#		for j in range(0, total_number_threads):
		
#	 		os.chdir(str(main_path) + "/parallel/" + str(folder_fault_model) + input_parameter + "_" + str(j))
#			cmd_bash_sim = "bash launch_zoixs.sh"
			
#			p = subprocess.Popen(cmd_bash_sim, stdout=subprocess.PIPE, shell=True)
#			(out, error) = p.communicate()
#			print(out)
#			print(error)

		# Method number two: multi-thread launcher...
			
		# A limit in the number of possible parallel thread is introduced to avoid the overhead in hard drive memory.
		
		max_current_threads = 20		# max limit of resident threads during the execution: 20 
		current_threads = 0
		limit_threads = 0
		all_threads_executed = 0		# init with all threads not executed

		limit_threads = current_threads + max_current_threads
		if limit_threads > total_number_threads:
			limit_threads = total_number_threads

		while all_threads_executed == 0:

			# Defining and restarting the status array:
			status_variables = []			# array of status variables
			
			# Routine to generate the files and folders for each individual campaign
			for ii in range (current_threads, (limit_threads), 1):
			
				# Copy/Generate the folder name
				os.mkdir(input_parameter + "_" + application_name + "_" + str(ii))
#				os.system("cp " + input_parameter + " " + input_parameter + "_" + application_name + "_" + str(ii) )					 # Copy the original files
				os.system("cp -r " + input_parameter + "/bin " + input_parameter + "_" + application_name + "_" + str(ii) + "/bin" )			 # Copy the original files
				os.system("cp -r " + input_parameter + "/DUT " + input_parameter + "_" + application_name + "_" + str(ii) + "/DUT" )			 # Copy the original files
				os.system("cp -r " + input_parameter + "/log " + input_parameter + "_" + application_name + "_" + str(ii) + "/log" )			 # Copy the original files
				os.system("cp " + input_parameter + "/clean.csh " + input_parameter + "_" + application_name + "_" + str(ii) + "/clean.csh")		 # Copy the original files
				os.system("cp " + input_parameter + "/launch_zoixs.sh " + input_parameter + "_" + application_name + "_" + str(ii) + "/launch_zoixs.sh") # Copy the original files

				# modify the strobes files to add the conditions of the start and ending points.
				modify_strobes_files(ii, analyzed_list[ii], input_parameter, application_name)
				print( "Generating the files for the simulation: " + str(ii) )
			
			# launching the simulations...
			for jj in range (current_threads, (limit_threads), 1):

		 		os.chdir(str(main_path) + "/parallel/" + str(folder_fault_model) + input_parameter + "_" + application_name + "_" + str(jj))

				cmd_bash_sim = "screen -dm bash -c 'bash launch_zoixs.sh' "	# effectively launched each one per independent thread

				status_variables.append(1)
			
				os.system(cmd_bash_sim)

				time.sleep(2)				# small delay to avoid reuse of resources...

				print( "launching the simulation " + str(jj) )
			
			#used to erase useless files
			os.chdir(str(main_path) + "/parallel/" + str(folder_fault_model))
						
			# ask if the files exist to know if the sim finished...
			running = 1
			while (running == 1):
		
				# check if the file exist...
				for kk in range (current_threads, (limit_threads), 1):
			
					if (os.path.isfile(str(main_path) + "/parallel/" + str(folder_fault_model) + input_parameter + "_" + application_name + "_" + str(kk) + "/saf_dic_long.txt" )):
						# disable the status flag when finished the thread:
						status_variables[kk - current_threads] = 0
	
				temp_status = 123
				for j in range (0, len(status_variables)):
					temp_status = status_variables[j] + temp_status
				
				if temp_status == 123:
					running = 0
					print("Fault sim finished...   (" + str(current_threads) + ") " + str(status_variables) + " (" + str( limit_threads ) + ")" )

					time.sleep(10)				# small delay to allow the writing of the results in the reports...
					
					# removing all useless simulation files after using and finding the expected dictionaries...
					for i in range(current_threads, (limit_threads), 1):
						os.system("rm -r " + input_parameter + "_" + application_name + "_" + str(i) + "/bin" )
						os.system("rm -r " + input_parameter + "_" + application_name + "_" + str(i) + "/DUT" )
						os.system("rm -r " + input_parameter + "_" + application_name + "_" + str(i) + "/__fmsh__" )
						os.system("rm -r " + input_parameter + "_" + application_name + "_" + str(i) + "/__fubs__" )
						os.system("rm -r " + input_parameter + "_" + application_name + "_" + str(i) + "/__globfiles__" )
						os.system("rm -r " + input_parameter + "_" + application_name + "_" + str(i) + "/log" )
						os.system("rm -r " + input_parameter + "_" + application_name + "_" + str(i) + "/sim.csrc" )
						os.system("rm -r " + input_parameter + "_" + application_name + "_" + str(i) + "/__tests__" )
						os.system("rm -r " + input_parameter + "_" + application_name + "_" + str(i) + "/__tmp__" )
						os.system("rm -r " + input_parameter + "_" + application_name + "_" + str(i) + "/zoix.sim.daidir" )
						os.system("rm " + input_parameter + "_" + application_name + "_" + str(i) + "/clean.csh" )
						os.system("rm " + input_parameter + "_" + application_name + "_" + str(i) + "/fault_report.log" )
						os.system("rm " + input_parameter + "_" + application_name + "_" + str(i) + "/fr2fdef.log" )
						os.system("rm " + input_parameter + "_" + application_name + "_" + str(i) + "/launch_zoixs.sh" )
						os.system("rm " + input_parameter + "_" + application_name + "_" + str(i) + "/logic_sim.log" )
						os.system("rm " + input_parameter + "_" + application_name + "_" + str(i) + "/sim.fdef" )
						os.system("rm " + input_parameter + "_" + application_name + "_" + str(i) + "/sim.zdb" )
						os.system("rm " + input_parameter + "_" + application_name + "_" + str(i) + "/testability.txt" )
						os.system("rm " + input_parameter + "_" + application_name + "_" + str(i) + "/zoix.sim" )

				else:
					running = 1
					print("Fault sim continues:  (" + str(current_threads) + ") " + str(status_variables) + " (" + str( limit_threads ) + ")")				
					time.sleep(10)				# small delay to avoid reuse of resources...
#			print("XXXX")
#			print(limit_threads, current_threads, max_current_threads)

			current_threads = current_threads + max_current_threads
			if ( (current_threads > total_number_threads) or (current_threads == total_number_threads) ):
				all_threads_executed = 1
...
#			print(limit_threads, current_threads, max_current_threads)
		...
		# then, use parallel threads and join all the reports as only one...
		
		# add the analysis and join function of all reports:
		collapse_reports(input_parameter, folder_fault_model, total_number_threads, application_name)
		
#		print(str(main_path) + "/parallel/" + str(folder_fault_model))
		...
		if os.path.isfile("saf_dic_long.txt"):
			...
			if os.path.isdir("./" + str(application_name)):
				...
				# moving the independent results for each instruction on the targeted app:

				os.system("mkdir " + str(application_name) + "/individual_ins")				
				for j in range(0, total_number_threads):
								
					os.system("mv saf_dic_long" + str(j) + ".txt " + str(application_name) + "/individual_ins/saf_dic_long" + str(j) + ".txt")
					os.system("mv ../" + input_parameter + "_" + application_name + "_" + str(j) + "/fmsh.log " + str(application_name) + "/individual_ins/fault_coverage_results" + str(j) + ".txt")
					os.system("mv ../" + input_parameter + "_" + application_name + "_" + str(j) + "/SAF_coverage.sff " + str(application_name) + "/individual_ins/SAF_coverage" + str(j) + ".sff")
...
#				os.system("mv SAF_coverage.sff " + str(application_name) + "/SAF_coverage.sff")
		...
		# removing all the simulation files after using and finding the expected dictionaries...
		for i in range(0, total_number_threads):
			os.system("rm -r " + input_parameter + "_" + application_name + "_" + str(i) )		# Copy the original files for the independent operation of each 
...
def	Select_application(application_name, File_path):
	...
	os.chdir(str(main_path) + "/intruction_level_ers/TB/applications/" + str(application_name))
...
# back_propagation, euler, FFT, gray_filter, mxm, mxm_simple, nn, reduction, scalar_vector_product, scan, sobel, sort, transpose, vector_add
...
#	input_parameter = "warp_unit" 				# replace for input argument
	...
	input_fault_model = "permanent"  			# There is also the chance to use transient and transition
	...
	print("Target to analyze...")

	# collection of the target module to analyze    input parameter

	File_path = os.getcwd()
	print(File_path)					# used to return to the original path

	print("Target application...")
	
	# Define the procedures to adjust the app for the golden simulation...
	
	Select_application(application_name, File_path)
	
	# Use input argument parameters to define the target app and also the target module.

	fault_injection_enable = 1			# controls the execution of the faul-injection tool.
	fault_analysis_enable = 1			# controls the execution of the fault-analysis tool.

	official_starting_time = 0
	official_kernel_op_time = 0
 

	print("golden simulation...")
	(fault_injection_enable, timestamp1, timestamp2, timestamp3, official_starting_time, official_kernel_op_time) = execute_golden_sim(input_parameter, input_fault_model)


#	print("Fault-injection campaign...")
	(timestamp4, timestamp5, fault_analysis_enable) = execute_fault_injection_campaign(fault_injection_enable, input_parameter, input_fault_model, application_name, official_starting_time, official_kernel_op_time)


	print("Fault classification...")
	(timestamp6, timestamp7) = execute_fault_classification(fault_analysis_enable, input_parameter, input_fault_model, application_name, official_starting_time, official_kernel_op_time)

	# Reporting the execution times of each step of the framework:
	
	golden_model_import_time = timestamp2 - timestamp1
	golden_simulation_time = timestamp3 - timestamp2
	total_golden_simulation_time = timestamp3 - timestamp1
	
	fault_injection_simulation_time = timestamp5 - timestamp4
	fault_analysis_time = timestamp7 - timestamp6
	
	general_output_report = open(input_parameter + "/" +  application_name + "/general_output_results.txt", 'a')
	
	print("execution times:")
	general_output_report.write("execution times:\n")
	print("Golden model import time(s): " + str(golden_model_import_time))
	general_output_report.write("Golden model import time(s): " + str(golden_model_import_time) + "\n")
	print("Golden simulation time(s): " + str(golden_simulation_time))
	general_output_report.write("Golden simulation time(s): " + str(golden_simulation_time) + "\n")
	print("Total golden simuation time(s): " + str(total_golden_simulation_time))
	general_output_report.write("Total golden simuation time(s): " + str(total_golden_simulation_time) + "\n")
	print("fault-injection simulation time(s): " + str(fault_injection_simulation_time))
	general_output_report.write("fault-injection simulation time(s): " + str(fault_injection_simulation_time) + "\n")
	print("fault analysis time(s): " + str(fault_analysis_time))
	general_output_report.write("fault analysis time(s): " + str(fault_analysis_time) + "\n")
	print("done........................................................................")
	general_output_report.write("done........................................................................\n")

	# removing temporal files fo the Zoix compilation:
	
	os.system("rm -r " + input_fault_model + "/" + input_parameter + "/" + "__fmsh__")
	os.system("rm -r " + input_fault_model + "/" + input_parameter + "/" + "__fubs__")
	os.system("rm -r " + input_fault_model + "/" + input_parameter + "/" + "__globfiles__")
	os.system("rm -r " + input_fault_model + "/" + input_parameter + "/" + "__tests__")
	os.system("rm -r " + input_fault_model + "/" + input_parameter + "/" + "__tmp__")
	os.system("rm -r " + input_fault_model + "/" + input_parameter + "/" + "sim.csrc")
	os.system("rm -r " + input_fault_model + "/" + input_parameter + "/" + "zoix.sim.daidir")
	os.system("rm " + input_fault_model + "/" + input_parameter + "/" + "fault_report.log")
	os.system("rm " + input_fault_model + "/" + input_parameter + "/" + "fr2fdef.log")
	os.system("rm " + input_fault_model + "/" + input_parameter + "/" + "logic_sim.log")
	os.system("rm " + input_fault_model + "/" + input_parameter + "/" + "sim.fdef")
	os.system("rm " + input_fault_model + "/" + input_parameter + "/" + "sim.zdb")
	os.system("rm " + input_fault_model + "/" + input_parameter + "/" + "zoix.sim")
