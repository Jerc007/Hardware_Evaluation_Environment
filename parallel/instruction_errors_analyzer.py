...
def search_inst(finel_name_inst, inst):
	
	input_filexxx = open(finel_name_inst, 'r')
	collected_inst_type = ""
	optional_par = " "
	inst = inst.replace(" ","")
	
	for linex in input_filexxx:
		wordsx = linex.split(" ")

		if inst == wordsx[0]:

			collected_inst_type = wordsx[1].replace("\n", "")

#			print(wordsx, linex, inst, wordsx[0], collected_inst_type)

			if len(wordsx) == 3:
				optional_par = wordsx[2]
			else:
				optional_par = " "
	
	input_filexxx.close()

	return(collected_inst_type, optional_par)
...
#	input_parameter = "warp_unit" 				# replace for input argument
	...
	address_file_name1 = fault_model + "/" + input_parameter + "/" + application_name + "/inst_classification_execution.txt"
	address_file_name2 = fault_model + "/" + input_parameter + "/" + application_name + "/inst_classification_operand.txt"
	address_file_name3 = fault_model + "/" + input_parameter + "/" + application_name + "/inst_classification_operational.txt"
	address_file_name4 = fault_model + "/" + input_parameter + "/" + application_name + "/inst_classification_order.txt"
	finel_name_inst = "ISA_types.txt"
	...
	filex_filex = [address_file_name1, address_file_name2, address_file_name3, address_file_name4]

	for i in filex_filex:
		
		input_file1 = open(i, 'r')
		...
		for line in input_file1:
			...
			(inst_type, op_param) = search_inst(finel_name_inst, inst)
	#		print(inst_type, op_param)

			if error == "incorrect_operation_code":
				decoded_error = "ICOC"

			if error == "invalid_op_code":
				decoded_error = "IVOC"

			if error == "invalid_register":
				decoded_error = "IR"

			if error == "incorrect_register_addressed":
				decoded_error = "IRA"

			if error == "incorrect_immediate_operand":
				decoded_error = "IIO"

			if error == "incorrect_parallel_parameters":
				decoded_error = "IPP"

			if error == "commitment_order_violation":
				decoded_error = "WV"

			if error == "incorrect_functional_unit_used":
				decoded_error = "IFU"

			if error == "incorrect_active_threads":
				decoded_error = "IAT"

			if error == "incorrect_active_warp":
				decoded_error = "IAW"

			if error == "incorrect_active_CTA":
				decoded_error = "IAC"

			if error == "incorrect_active_line":
				decoded_error = "IAL"

			if error == "incorrect_memory_source":
				decoded_error = "IMS"

			if error == "incorrect_memory_destiny":
				decoded_error = "IMD"

			if error == "hanging":
				decoded_error = "hanging"


			if decoded_error in int_errors:
				int_errors[decoded_error] = int_errors[decoded_error] + int(words[2])
			else:
				int_errors[decoded_error] = int(words[2])

			total_errors = total_errors + int(words[2])

#			print("xxxxxxx")
			print(line, inst_type, decoded_error, int(words[2]))
#			print(".............before.............")
#			print(instruction_types)
#			print("................................")

			if inst_type in instruction_types:
				if decoded_error in instruction_types[inst_type]:
					instruction_types[inst_type][decoded_error] = instruction_types[inst_type][decoded_error] + int(words[2])
				else:
					instruction_types[inst_type][decoded_error] = int(words[2])
			else:
				temp_dict = dict()
				temp_dict[decoded_error] = int(words[2])
				instruction_types[inst_type] = temp_dict

#			print(instruction_types)
#			print("................after................")

	# split in the error groups:
	operation = 0
	operand = 0
	order = 0
	execution = 0
	...
	for key in int_errors:

		if key == "WV":
			order = order + int_errors[key]
		elif key == "ICOC":
			operation = operation + int_errors[key]
		elif key == "IVOC":
			operation = operation + int_errors[key]
		elif key == "IRA":
			operand = operand + int_errors[key]
		elif key == "IR":
			operand = operand + int_errors[key]
		elif key == "IIO":
			operand = operand + int_errors[key]
			...
			execution = execution + int_errors[key]
		elif key == "IAT":
			execution = execution + int_errors[key]
		elif key == "IAC":
			execution = execution + int_errors[key]
			...
			hanging = hanging+ int_errors[key]


	print( int_errors )
	print( "print total errors: " + str(total_errors) )
...
# todo: build the function to analyze and divide the dictionary into the four main types of errors for the graph...
	...
	for key in instruction_types:
		...
		if key == "A_L":
			...
			for internal_key in instruction_types[key]:

				if internal_key == "WV":
					A_L_order = A_L_order + instruction_types[key][internal_key]
				elif internal_key == "ICOC":
					A_L_operation = A_L_operation + instruction_types[key][internal_key]
				elif internal_key == "IVOC":
					A_L_operation = A_L_operation + instruction_types[key][internal_key]
				elif internal_key == "IRA":
					A_L_operand = A_L_operand + instruction_types[key][internal_key]
				elif internal_key == "IR":
					A_L_operand = A_L_operand + instruction_types[key][internal_key]
				elif internal_key == "IIO":
					A_L_operand = A_L_operand + instruction_types[key][internal_key]
				elif internal_key == "IPP":
					A_L_operand = A_L_operand + instruction_types[key][internal_key]
				elif internal_key == "IFU":
					A_L_execution = A_L_execution + instruction_types[key][internal_key]
				elif internal_key == "IAT":
					A_L_execution = A_L_execution + instruction_types[key][internal_key]
				elif internal_key == "IAC":
					A_L_execution = A_L_execution + instruction_types[key][internal_key]
				elif internal_key == "IAL":
					A_L_execution = A_L_execution + instruction_types[key][internal_key]
				elif internal_key == "IAW":
					A_L_execution = A_L_execution + instruction_types[key][internal_key]
				elif internal_key == "IMS":
					A_L_execution = A_L_execution + instruction_types[key][internal_key]
				elif internal_key == "IMD":
					A_L_execution = A_L_execution + instruction_types[key][internal_key]
				elif internal_key == "hanging":
					A_L_hanging = A_L_hanging+ instruction_types[key][internal_key]
			...
			for internal_key in instruction_types[key]:
				...
				if internal_key == "WV":
					DH_M_order = DH_M_order + instruction_types[key][internal_key]
				elif internal_key == "ICOC":
					DH_M_operation = DH_M_operation + instruction_types[key][internal_key]
				elif internal_key == "IVOC":
					DH_M_operation = DH_M_operation + instruction_types[key][internal_key]
				elif internal_key == "IRA":
					DH_M_operand = DH_M_operand + instruction_types[key][internal_key]
				elif internal_key == "IR":
					DH_M_operand = DH_M_operand + instruction_types[key][internal_key]
				elif internal_key == "IIO":
					DH_M_operand = DH_M_operand + instruction_types[key][internal_key]
					...
					DH_M_execution = DH_M_execution + instruction_types[key][internal_key]
				elif internal_key == "IAT":
					DH_M_execution = DH_M_execution + instruction_types[key][internal_key]
				elif internal_key == "IAC":
					DH_M_execution = DH_M_execution + instruction_types[key][internal_key]
					...
					DH_M_hanging = DH_M_hanging+ instruction_types[key][internal_key]
			...
			for internal_key in instruction_types[key]:
				...
				if internal_key == "WV":
					CF_order = CF_order + instruction_types[key][internal_key]
				elif internal_key == "ICOC":
					CF_operation = CF_operation + instruction_types[key][internal_key]
				elif internal_key == "IVOC":
					CF_operation = CF_operation + instruction_types[key][internal_key]
				elif internal_key == "IRA":
					CF_operand = CF_operand + instruction_types[key][internal_key]
				elif internal_key == "IR":
					CF_operand = CF_operand + instruction_types[key][internal_key]
				elif internal_key == "IIO":
					CF_operand = CF_operand + instruction_types[key][internal_key]
					...
					CF_execution = CF_execution + instruction_types[key][internal_key]
				elif internal_key == "IAT":
					CF_execution = CF_execution + instruction_types[key][internal_key]
				elif internal_key == "IAC":
					CF_execution = CF_execution + instruction_types[key][internal_key]
					...
					CF_hanging = CF_hanging+ instruction_types[key][internal_key]

	
	print("A_L_operation " + str(A_L_operation))
	print("A_L_operand " + str(A_L_operand))
	print("A_L_order " + str(A_L_order))
	print("A_L_execution " + str(A_L_execution))
	print("A_L_hanging " + str(A_L_hanging))

	print("DH_M_operation " + str(DH_M_operation))
	print("DH_M_operand " + str(DH_M_operand))
	print("DH_M_order " + str(DH_M_order))	
	print("DH_M_execution " + str(DH_M_execution))
	print("DH_M_hanging " + str(DH_M_hanging))

	print("CF_operation " + str(CF_operation))
	print("CF_operand " + str(CF_operand))
	print("CF_order " + str(CF_order))
	print("CF_execution " + str(CF_execution))
	print("CF_hanging " + str(CF_hanging))
