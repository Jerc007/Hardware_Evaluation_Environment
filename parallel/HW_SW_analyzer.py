...
#	input_parameter = "warp_unit" 				# replace for input argument
	...
	address_file_name = fault_model + "/" + input_parameter + "/" + application_name + "/" +  input_parameter + "_report_Hw_faults_errors_relation.txt"
	...
	for line in input_file:
		if ("operation" in line) or ("operand" in line) or ("order" in line) or ("execution" in line) or ("operation" in line):
			words = line.split(" ")
			soft_errors = int(words[1])
			hard_faults = int(words[2])
			
			total_HW = total_HW + hard_faults
			total_SW = total_SW + soft_errors
			
	print("total HW faults = " + str(total_HW) )
	print("total SW errors = " + str(total_SW) )
